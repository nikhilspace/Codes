...
void ad() 
{
    ...
    while(fscanf(old,"%s %s %llu %s %s %d/%d/%d \n",add.book,add.genre,&check.book_id,add.status,add.borrower_name,&add.last_update.day,&add.last_update.month,&add.last_update.year)!=EOF) 
    {
        if (add.book_id==check.book_id) 
        { 
